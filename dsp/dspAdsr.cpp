...
*	@param	outVal initial output value
*	@param	minimumval minimum allowed output value
...
*/
float DSP_Adsr::set_attack_time_sec(int at_sec)
{
...
*	@return Attack time set value in sec
*/
float DSP_Adsr::set_decay_time_sec(int dc_sec)
{
...
*	@return Attack time set value in sec
*/
float DSP_Adsr::set_release_time_sec(int dc_sec)
{
...
float DSP_Adsr::calc_next_envelope_val() {
	...
	static float decayOffLevel = 0.0f;
	...
	case ADSR_STATE_SUSTAIN:
		// No change, unless very small, than release 
	...
	case ADSR_STATE_OFF_RELEASE:
		if (decayOffLevel == 0)
		{
			decayOffLevel = output_val;
		}
			
		if ((output_val <= release_step*decayOffLevel) || (decayOffLevel <= 0.1))
		{
			output_val = 0.0f;
			decayOffLevel = 0.0f;
			set_state(ADSR_STATE_IDLE);
		...
		{
			output_val -= release_step*decayOffLevel;
		}
 ...
 *	@return: none
 */
void DSP_Adsr::set_note_off() { note_on = false; }

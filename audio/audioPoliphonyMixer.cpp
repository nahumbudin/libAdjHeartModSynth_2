...
*
*	@brief		Mix audio ch1 and ch2 of all voicesinto stereo Left and Right output signals 
*/
		...
		stage)
{
	...
	if (inputs > mod_synth_get_synthesizer_num_of_polyphonic_voices())
	{	
		inputs = mod_synth_get_synthesizer_num_of_polyphonic_voices();
	}
	...
	{
		program_level_1[program] = program_level_2[program] = 0.5f;
		program_pan_1[program] = program_pan_2[program] = 0.0f;
		program_send_1[program] = program_send_2[program] = 0.0f;
	}
	...
	AudioBlockFloat** audio_first_update_ptr)
{
	if (audio_poly_mixer_instance == NULL)
	{
		audio_poly_mixer_instance = new AudioPolyMixerFloat(
			stage, num_of_voices, block_size, mapping_mode,
			audio_first_update_ptr);
	}
...
*   @return buffer size
*/	
int AudioPolyMixerFloat::get_audio_block_size() {	return audio_block_size; }
...
*   @return buffer size
*/	
int AudioPolyMixerFloat::get_midi_maping_mode() {	return midi_mapping_mode; }
...
{
	if ((voice >= 0) && (voice < mod_synth_get_synthesizer_max_num_of_polyphonic_voices()))
	{
...
void AudioPolyMixerFloat::set_voice_gain_1_ptr(int voice, int prog)
{
	if ((voice >= 0) && (voice < mod_synth_get_synthesizer_num_of_polyphonic_voices()) &&
		(prog >= 0) && (prog < mod_synth_get_synthesizer_num_of_programs()))
	{

...
#define _MIDI_TILTUNE_SYSEX_VENDOR_ID_1			0x35 
...
#define _MIDI_STAGE_0							0
#define _MIDI_STAGE_1							1
...
typedef	struct 
{
	...
	union 
	{
		...
		uint8_t bytes[_MIDI_MSSG_MAX_LEN/*5*/];
		
		// Used for pointing a raw data message		
////		raw_data_mssg_block_t *dataptr; 
	} data_t;
...
public: 
	...
	MidiConnection(MidiStream *source,
		unsigned char source_output,
		MidiStream *destination,
		unsigned char destination_input);
...
#define allocate_midi_stream_messages_memory_pool(num) ({ \
	if (num > _MAX_MIDI_STREAM_MESSAGES_POOL_SIZE) { \
		...
		MidiStream::initialize_midiStream_memory(data, _MAX_MIDI_STREAM_MESSAGES_POOL_SIZE); \
	} \
	else { \
		static midi_stream_mssg_block_t data[num]; \
		MidiStream::initialize_midiStream_memory(data, num); \
	} \
} )
...
#define allocate_raw_data_mssgs_memory_pool(num) ({ \
	if (num > _MAX_RAWDATA_MSSGS_POOL_SIZE) { \
		...
		MidiStream::initialize_rawdataMssgs_memory(data, _MAX_RAWDATA_MSSGS_POOL_SIZE); \
	} \
	else { \
		static raw_data_mssg_block_t data[num]; \
		MidiStream::initialize_rawdataMssgs_memory(data, num); \
	} \
} )
...
public:
	...
	static void initialize_midiStream_memory(midi_stream_mssg_block_t *data, unsigned int num);
	static void initialize_rawdataMssgs_memory(raw_data_mssg_block_t *data, unsigned int num);

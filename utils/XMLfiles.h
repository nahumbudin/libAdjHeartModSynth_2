...
*	History:\n
*
*	version 1.0		17-Oct-2019:	First version
*/
...
public: 
	...
	*/
	int read_xml_file(const char *full_path = NULL, std::string *file_data = NULL);
	/**
	* write an vector<string> XML elements text filedata into an XML file (removes file extension and adds .xml)
	*/
	int write_xml_file(const char *full_path = NULL, vector<string> *file_data = NULL);
	/**
	* Extract file name from full path 
	*/
	std::string get_xml_file_name(std::string full_path);
	/**
...
private:
	...
	*/
	...
	/**
	*	Returns a vector of std::string elements of a given tag found in xml_text
	*/
	vector<std::string> get_elements(const std::string &xml_text, std::string tag);
};

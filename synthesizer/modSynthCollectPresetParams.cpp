/**
*	@file		modSynthCollectPresetParams.h
*	@author		Nahum Budin
*	@date		7-Feb-2021
*	@version	1.1
...
int ModSynth::collect_mod_synth_preset_parms(_setting_params_t *params) {
	...
	for (int i = 0; i < 16; i++)
	{
		...
		res |= AdjSynth::get_instance()->adj_synth_settings_manager->set_string_param_value(
			...
			fluid_synth->get_fluid_synth_channel_preset_name(i),
			_SET_VALUE,
			-1);
	...
	result = mod_synth->fluid_synth_settings_manager->get_bool_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.reverb.active", &bool_param);
	...
	result = mod_synth->fluid_synth_settings_manager->get_float_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.reverb.room-size", &float_param);
	if (result == _SETTINGS_KEY_FOUND)
	{
	...
	result = mod_synth->fluid_synth_settings_manager->get_float_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.reverb.damp", &float_param);
	if (result == _SETTINGS_KEY_FOUND)
	{
		float_val = float_param.value;
	}
	...
	result = mod_synth->fluid_synth_settings_manager->get_float_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.reverb.width", &float_param);
	if (result == _SETTINGS_KEY_FOUND)
	{
		float_val = float_param.value;
	}
	...
	result = mod_synth->fluid_synth_settings_manager->get_float_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.reverb.level", &float_param);
	if (result == _SETTINGS_KEY_FOUND)
	{
		float_val = float_param.value;
	}
	...
	result = mod_synth->fluid_synth_settings_manager->get_bool_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.chorus.active", &bool_param);
	if (result == _SETTINGS_KEY_FOUND)
	{
		bool_val = bool_param.value;
	}
	...
	result = mod_synth->fluid_synth_settings_manager->get_int_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.chorus.nr", &int_param);
	if (result == _SETTINGS_KEY_FOUND)
	{
	...
	result = mod_synth->fluid_synth_settings_manager->get_float_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.chorus.level", &float_param);
	if (result == _SETTINGS_KEY_FOUND)
	{
		float_val = float_param.value;
	}
	...
	result = mod_synth->fluid_synth_settings_manager->get_float_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.chorus.speed", &float_param);
	if (result == _SETTINGS_KEY_FOUND)
	{
		float_val = float_param.value;
	}
	...
	result = mod_synth->fluid_synth_settings_manager->get_float_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.chorus.depth", &float_param);
	if (result == _SETTINGS_KEY_FOUND)
	{
		float_val = float_param.value;
	}
	...
	result = mod_synth->fluid_synth_settings_manager->get_int_param(mod_synth->get_fluid_synth_active_settings_params(), "fluid.synth.chorus.waveform", &int_param);
	if (result == _SETTINGS_KEY_FOUND)
	{
		int_val = int_param.value;
	}

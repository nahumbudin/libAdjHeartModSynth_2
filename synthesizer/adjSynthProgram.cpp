	...
	AudioManager *aud_mng)
{
	...
	active_patch_params.settings_type = _PARAM_TYPE_ADJ_SYNTH_PATCH;
	settings_manager = new ModSynthSettings(&active_patch_params);
...
*/
int SynthProgram::activate_set_patch_settings_default_params_callback(_setting_params_t* params, int prog)
{
	if (set_patch_settings_default_params_callback_ptr)
	{
...
*   @return void
*/
void SynthProgram::set_program_patch_params(_setting_params_t *params)
{
	settings_res_t res;
	...
	for(std::map<std::string,
		mod_synth_settings_str_param_t>::iterator param = params->string_parameters_map.begin();
	...
	for(std::map<std::string,
		mod_synth_settings_int_param_t>::iterator param = params->int_parameters_map.begin();
	...
	for(std::map<std::string,
		mod_synth_settings_float_param_t>::iterator param = params->float_parameters_map.begin();
	...
	for(std::map<std::string,
		mod_synth_settings_bool_param_t>::iterator param = params->bool_parameters_map.begin();
...
{	
	...
	if ((nov > 0) && (nov <= mod_synth_get_synthesizer_num_of_polyphonic_voices()))
	{
		num_of_voices = nov;
